...
name_to_model = {}
bench = ImageBenchmark(datasets_dir="../data", models_dir="../models")
...
def compare_with_adv(model1, model2, truth=-1):
    ...
    if args.profiling_mode == "hybrid":
        adv_inputs2 = gen_adv_inputs(model2.torch_model, seed_inputs2)
        hybrid_inputs = np.concatenate([seed_inputs2, adv_inputs2])
    ...
    elif args.cmp_mode == "weight":
        sim = md.compute_similarity_with_weight()
    elif args.cmp_mode == "whole_weight":
        sim = md.compute_similarity_with_whole_weight()
    elif args.cmp_mode == "feature":
        sim = md.compute_similarity_with_feature(inputs)
    elif args.cmp_mode == "lastfeature":
        sim = md.compute_similarity_with_last_feature(inputs)
    elif args.cmp_mode == "lastfeaturesvd":
        sim = md.compute_similarity_with_last_feature_svd(inputs)
    elif args.cmp_mode == "identicalweight":
        sim = md.compute_similarity_with_identical_weight()
    elif args.cmp_mode == "bnweight":
        sim = md.compute_similarity_with_bn_weight()
    elif args.cmp_mode == "convbnweight":
        sim = md.compute_similarity_with_conv_bn_weight()
    elif args.cmp_mode == "absweight":
        sim = md.compute_similarity_with_abs_weight()
